...
public class OrderAggregate {
    ...
    public void apply(String type, int schemaVersion, Map<String, Object> payload) {
        if (schemaVersion == 0) {
            ...
            if (payload.containsKey("customerId")) payload.put("customer", payload.remove("customerId"));
        ...
        switch (type) {
            ...
            case "Aborting" -> {
                status = "COMPENSATING";
                data.put("reason", payload.get("reason"));
            }
            case "Confirmed" -> {
                if (!status.equals("PENDING")) throw new IllegalStateException("Confirmation after abort");
                status = "CONFIRMED";
            }
    ...
    public boolean aborting() {
        return Set.of("COMPENSATING", "MANUAL_RECOVERY", "CANCELLED", "FAILED").contains(status);
    }

    public boolean terminal() {
        return Set.of("CONFIRMED", "CANCELLED", "FAILED").contains(status);
    }

    public Map<String, Object> view(UUID id) {
        var p = new LinkedHashMap<>(data);
        p.put("orderId", id.toString());
        p.put("status", status);
        p.put("version", version);
        p.put("progress", facts.stream().sorted().toList());
        return p;
    }
}
