...
public class OrderStore {
    ...
    public OrderStore(Database db, Messaging messaging) {
        this.db = db;
    ...
    public OrderAggregate load(UUID id, boolean snapshots) {
        ...
        if (snapshots) {
            var rows = db.sql.queryForList("select * from order_snapshot where aggregate_id=? and schema_version=1", id);
        ...
        for (var row : db.sql.queryForList("select * from domain_event where aggregate_id=? and version>? order by version", id, a.version)) {
            if (((Number) row.get("version")).longValue() != a.version + 1)
                throw new IllegalStateException("Corrupt event stream");
    ...
    public void append(UUID id, OrderAggregate a, String type, Map<String, Object> payload, Event cause) {
        if (db.sql.update("update order_stream set version=version+1 where id=? and version=?", id, a.version) != 1)
            throw new OptimisticLockingFailureException("Stale aggregate version");
        ...
        if (a.version % 5 == 0)
            db.sql.update("insert into order_snapshot values(?,?,1,?::jsonb) on conflict(aggregate_id) do update set version=excluded.version,body=excluded.body,schema_version=1", id, a.version, Json.write(a));
        ...
        if (Set.of("Placed", "Aborting", "Confirmed").contains(type))
            messaging.publish(Event.next("Order" + type, id, a.version, "order-service", a.view(id), cause));
    }
}
