...
public class PaymentParticipant implements EventHandler {
    ...
    public PaymentParticipant(Database db, Messaging messaging, ProviderClient provider, MeterRegistry metrics) {
        this.db = db;
    ...
    public void handle(Event e) {
        ...
        if (e.eventType().equals("InventoryReserved") && !e.producer().equals("inventory-service"))
        ...
        if (e.eventType().equals("OrderAborting")) {
            if (rows.isEmpty()) {
                ...
                emit(e.aggregateId(), "PaymentVoided", e.payload(), e);
            } else {
                ...
                if (state.equals("REJECTED")) {
                    ...
                    emit(e.aggregateId(), "PaymentVoided", Json.map(row.get("body").toString()), e);
                }
            }
    ...
    public void process(UUID id) {
        ...
        db.transaction(() -> {
            ...
            try {
                var result = provider.call("payment", id.toString(), null);
                ...
                if (remote.equals("ABSENT")) {
                    ...
                    result = provider.call("payment", id.toString(), Map.of("mode", body.get("paymentMode"), "request", body));
                    remote = result.get("state").toString();
                }
                if (remote.equals("REJECTED")) {
                    finish(id, aborted ? "COMPENSATED" : "REJECTED", aborted ? "PaymentVoided" : "PaymentRejected", body);
                    return null;
                }
                ...
                if (aborted) {
                    if (!remote.equals("COMPENSATED"))
                        provider.call("payment/compensate", id.toString(), Map.of("mode", body.get("paymentMode")));
                    finish(id, "COMPENSATED", "PaymentVoided", body);
    ...
    private void emit(UUID id, String type, Map<String, Object> body, Event cause) {
        ...
        messaging.publish(Event.next(type, id, version, "payment-service", body, cause));
    }
