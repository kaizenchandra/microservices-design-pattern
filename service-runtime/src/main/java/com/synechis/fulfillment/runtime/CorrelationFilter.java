...
public class CorrelationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws java.io.IOException, ServletException {
        ...
        if (trace == null || !trace.matches("00-[a-f0-9]{32}-[a-f0-9]{16}-0[01]"))
            trace = "00-" + UUID.randomUUID().toString().replace("-", "") + "-" + UUID.randomUUID().toString().replace("-", "").substring(0, 16) + "-01";
        if (tracer != null && tracer.currentSpan() != null) {
            var context = tracer.currentSpan().context();
            trace = "00-" + context.traceId() + "-" + context.spanId() + "-01";
        }
        Correlation.set(Map.of("correlationId", id, "traceparent", trace));
