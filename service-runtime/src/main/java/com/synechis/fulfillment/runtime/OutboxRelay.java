...
public class OutboxRelay {
    ...
    public OutboxRelay(Database db, KafkaTemplate<String, String> kafka, MeterRegistry metrics) {
        this.db = db;
        this.kafka = kafka;
    ...
    @Scheduled(fixedDelayString = "${app.relay.delay:100}")
    public void tick() {
        if (enabled) for (int i = 0; i < 32; i++) if (!relayOne(false)) break;
    }

    public boolean relayOne(boolean crashAfterSend) {
        return db.transaction(() -> {
            ...
            try (var scope = tracer == null || span == null ? null : tracer.withSpan(span)) {
                ...
                db.sql.update("update outbox set published_at=now() where id=?", id);
    ...
    public void cleanup() {
        db.sql.update("delete from outbox where published_at<now()-interval '7 days'");
    }
