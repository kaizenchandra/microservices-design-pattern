...
public class ProviderClient {
 ...
 private final String base;private final String key;
 public ProviderClient(@Value("${PROVIDER_URL:http://localhost:8090}") String base,@Value("${PROVIDER_KEY:local-simulator-only}") String key,MeterRegistry metrics){this.base=base;this.key=key;metrics.gauge("provider.circuit.state",breaker,b->b.getState().getOrder());}
 public Map<String,Object> call(String operation,String id,Map<String,Object> body){
  return Bulkhead.decorateSupplier(bulkhead,CircuitBreaker.decorateSupplier(breaker,()->{
   try{
    ...
    if(response.statusCode()!=200)throw new IllegalStateException("Provider outcome unknown; HTTP "+response.statusCode());
    return Json.map(response.body());
   }catch(InterruptedException e){Thread.currentThread().interrupt();throw new IllegalStateException("Interrupted provider call",e);}catch(java.io.IOException e){throw new IllegalStateException("Provider outcome unknown",e);}
  })).get();
 }
}
