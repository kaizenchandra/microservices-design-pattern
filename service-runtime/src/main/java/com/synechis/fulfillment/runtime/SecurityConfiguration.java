...
public class SecurityConfiguration {
    private static void problem(jakarta.servlet.http.HttpServletResponse response, int status) throws java.io.IOException {
        response.setStatus(status);
        response.setContentType("application/problem+json");
        response.getWriter().write(com.synechis.fulfillment.contracts.Json.write(java.util.Map.of("type", "about:blank", "status", status, "title", status == 401 ? "Unauthorized" : "Forbidden")));
    }
